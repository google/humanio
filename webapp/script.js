...
async function runEverything(){
    ...
    // ACTIVITY Prediction based on detected objects, holding object, and image caption
    let promptActivity = prompts.ACTIVITY_PROMPT_PREFIX + visualInfo.IMAGE_CAPTION + prompts.ACTIVITY_PROMPT_SUFFIX;
    ...
    // ENVIRONMENT Prediction based on detected objects, holding object, and image caption
    let promptEnvironment = prompts.ENVIRONMENT_PROMPT_PREFIX + visualInfo.IMAGE_CAPTION + prompts.ENVIRONMENT_PROMPT_SUFFIX;
...
Q: ${activityResult} ${environmentResult}`;
    ...
    promptSituation += " The environmental volume level is around" + audioInfo.VOLUME + " decibels. ";
    promptSituation += prompts.SITUATION_PROMPT_SUFFIX; // comment out if using SITUATION_PROMPT_PREFIX_SHORT
    ...
    // for all statusDive, color based on status, Available -> green, Affected -> orange, Not Available -> red
    let statusDivs = [eyeStatusDiv, hearingStatusDiv, vocalStatusDiv, handStatusDiv];
...
document.getElementById("saveLogs").addEventListener("click", function(){
    ...
    csv += "timestamp,video_id,objects,hand_detected,holding,caption,hand_caption,volume,audio_classification,activity,environment,start_time,delay,video_time,eye_status,hearing_status,vocal_status,hand_status\n";
    for (let i = 0; i < logs.length; i++) {
        csv += logs[i].timestamp + "," + 
               logs[i].video_id + "," + 
               logs[i].objects + "," + 
               logs[i].hand_detected + "," + 
               logs[i].holding + "," + 
               logs[i].caption + "," + 
               logs[i].hand_caption + "," + 
               logs[i].volume + "," + 
               logs[i].audio_classification + "," + 
               logs[i].activity + "," + 
               logs[i].environment + "," + 
               logs[i].start_time + "," + 
               logs[i].delay + "," + 
...
// run runEverything() for the first time after X second, then every INTERVAL second
