...
// check if is webcam or loaded video
...
async function runStreamingAudioClassification() {
    ...
    scriptNode.onaudioprocess = function (audioProcessingEvent) {
        ...
        for (let i = 0; i < 3; i++) {
            // audioInfo.AUDIO_CLASSIFICATION.push(categories[i].categoryName + " " + (categories[i].score*100).toFixed(3).toString() + "%");
...
async function runVideoAudioClassification() {
    ...
    if (loadedVideo) {
        // Check if the srcObject property is not null before calling getAudioTracks()
...
async function runStreamingVolumeMeter() {
    ...
    .then(function(stream) {
        ...
        // create a ScriptProcessorNode for direct script processing of audio data
        let scriptProcessor = audioContext.createScriptProcessor(2048, 1, 1);

        // connect the microphone to the script processor
        microphone.connect(scriptProcessor);

        // connect the script processor to the AudioContext destination (e.g. speakers)
        scriptProcessor.connect(audioContext.destination);

        // set up an event listener for the script processor's onaudioprocess event
...
function runVideoVolumeMeter() {
    ...
    const getNormalizedVolumeLevel = () => {
        ...
        let avgVolume = volumes.reduce((a, b) => parseFloat(a) + parseFloat(b), 0) / volumes.length;
...
var toggleAudioClassification = direct.add({Audio_Classification: true}, 'Audio_Classification');

// add listener to toggle
toggleAudioClassification.onChange(function(value) {
    let audioClassificationResult = document.getElementById("audioClassificationResult");
    if (value) {
