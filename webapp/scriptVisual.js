...
import { ObjectDetector, HandLandmarker, FilesetResolver } from "https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision";
...
// GLOBAL VARIABLES
// create a class of global variables for another js file to use
class VisualInfo {
    constructor() {
...
async function runObjectDetection() {
    ...
    for (let n = 0; n < predictions.length; n++) {
        ...
        p.innerText =
            predictions[n].categories[0].categoryName + " " + 
                Math.round(parseFloat(predictions[n].categories[0].score) * 100) +
                "%";
        visualInfo.ALL_DETECTED_OBJECTS.push(p.innerText);
        p.style.left = `${predictions[n].boundingBox.originX * (video.offsetWidth/video.videoWidth) + video.offsetLeft}px`;
        p.style.top = `${predictions[n].boundingBox.originY * (video.offsetHeight/video.videoHeight) + video.offsetTop}px`;
        p.style.width = `${predictions[n].boundingBox.width * (video.offsetWidth/video.videoWidth)}px`;

        const highlighter = document.createElement("div");
        highlighter.setAttribute("class", "highlighterObjectDetection");
        highlighter.style.left = `${predictions[n].boundingBox.originX * (video.offsetWidth/video.videoWidth) + video.offsetLeft}px`;
        highlighter.style.top = `${predictions[n].boundingBox.originY * (video.offsetHeight/video.videoHeight) + video.offsetTop}px`;
        highlighter.style.width = `${predictions[n].boundingBox.width * (video.offsetWidth/video.videoWidth)}px`;
        highlighter.style.height = `${predictions[n].boundingBox.height * (video.offsetHeight/video.videoHeight)}px`;
        ...
        // if toggle button not on, hide the objects
...
async function runHandLandmarker() {
    ...
    if (results.landmarks.length > 0) {
        ...
        for (const landmarks of results.landmarks) {
            ...
            // HOLDING DETECTION
            // calculate the avereage distance between the thumb and the index, middle, ring and pinky finger
            let thumbIndex = distance(landmarks[4].x, landmarks[4].y, landmarks[8].x, landmarks[8].y);
            let thumbMiddle = distance(landmarks[4].x, landmarks[4].y, landmarks[12].x, landmarks[12].y);
            let thumbRing = distance(landmarks[4].x, landmarks[4].y, landmarks[16].x, landmarks[16].y);
            let thumbPinky = distance(landmarks[4].x, landmarks[4].y, landmarks[20].x, landmarks[20].y);
            let averageThumbDistance = (thumbIndex + thumbMiddle + thumbRing + thumbPinky) / 4;

            // loop through all detected objects and check if the hand has overlapped with an object
            for (let i = 0; i < objectHighlighters.length; i++) {
                // get the bounding box of the object
                let objectBoundingBox = objectHighlighters[i].getBoundingClientRect();
                // get the distance between the center of the hand and the center of the object
                let handCenterX = (landmarks[0].x + landmarks[9].x) / 2;
                let handCenterY = (landmarks[0].y + landmarks[9].y) / 2;
                let objectCenterX = objectBoundingBox.left + objectBoundingBox.width / 2;
                let objectCenterY = objectBoundingBox.top + objectBoundingBox.height / 2;
                let distanceBetweenCenters = distance(handCenterX, handCenterY, objectCenterX, objectCenterY);
                // console.log("distanceBetweenCenters: " + objectHighlighters[i].innerText + " " + distanceBetweenCenters); 
                // console.log("objectBoundingBox.width: " + objectBoundingBox.width)
                // check if the hand is close enough to the object, and if the thumb is closed, if the object is small enough, and the object is not a person
                if (distanceBetweenCenters < 1000 &&  averageThumbDistance < 0.25 && !objectHighlighters[i].innerText.includes("person")) {
...
async function runImageCaption() {
    ...
    // Set the canvas dimensions to match the video dimensions, resized by RESIZE_FACTOR
    canvas.width = video.videoWidth * RESIZE_FACTOR;
    canvas.height = video.videoHeight * RESIZE_FACTOR;

    // Draw the current video frame onto the canvas, resized by RESIZE_FACTOR
    context.drawImage(video, 0, 0, canvas.width, canvas.height);

    // Get the image data from the canvas, resized by RESIZE_FACTOR
    ...
    //  const handCaptionResult = await callGPTLite(`An egocentric view of C is showing ${captionResult}. What are C's hands likely doing in this situation?`);
    //  const handCaptionResultDiv = document.getElementById("captionHandResult");
    ...
    const handCaptionResult = await replicateGenerateCaption(imgDataUrl, false, "What are the hands doing?");
...
// helper function to calculate the distance between two hand landmarks
...
async function startCam(){
    ...
    // Hide the videoloaded and show the webcam.
...
async function loadVideo() {
    ...
    const videoUrl = "videos/" + document.getElementById("video-url").value + ".mp4";
    ...
    // Hide the webcam video element and show the loaded video element
    videoWebcam.style.display = "none";
    videoLoaded.style.display = "block";
  
    // Create a blob URL if the provided URL is a local file
    if (videoUrl.startsWith("blob:") || videoUrl.startsWith("http") || videoUrl.startsWith("https")) {
    ...
    // Add event listeners to process the loaded video
...
function setCorrectSize(){
    ...
    // set liveView as the same size as the video
    liveView.style.width = video.offsetWidth + "px";
    liveView.style.height = video.offsetHeight + "px";
    // set output_canvas as the same size as the video
...
// context.open();
// var toggleActivityRecognition = context.add({Activity_Recognition: true}, 'Activity_Recognition');
// var toggleEnvironmentRecognition = context.add({Environment_Recognition: true}, 'Environment_Recognition');
...
direct.open();
var toggleObjectDetection = direct.add({Object_Detection: true}, 'Object_Detection');
var toggleHandLandmarker = direct.add({Hand_Landmarker: true}, 'Hand_Landmarker');
var toggleContextResults = direct.add({Context_Results: true}, 'Context_Results');
